...
// Realistic Indian student data templates
const STUDENTS_RAW = [
...
// Helper to generate unique uid matching index
const getUid = (index) => `student_seeded_uid_${index + 1}`;
const getReportId = (index) => `report_seeded_id_${index + 1}`;

// Clean collections helper
...
const runSeeder = async () => {
  console.log("=== Starting Firestore Demo Seeder ===");

  try {
    // Authenticate as Admin Demo user
    ...
    await firestoreCustomClear('career_predictions');
    await firestoreCustomClear('ai_reports');
    ...
    for (let i = 0; i < STUDENTS_RAW.length; i++) {
      ...
      const academicPerf = Math.round(raw.cgpa);
...
`;
      ...
      console.log(`Seeded student ${i + 1}/${STUDENTS_RAW.length}: ${raw.name}`);
    }

    // Seed admin user just in case
    const adminUid = 'admin_seeded_uid_100';
    await setDoc(doc(db, 'users', adminUid), {
      ...
      interests: ["Engineering Education"],
      cgpa: 10.0,
      careerGoal: "Placement Coordinator",
      roadmapProgress: 100,
      role: "admin",
      createdAt: new Date().toISOString(),
...
// Helper for cleaning custom collections that may not exist yet
async function firestoreCustomClear(colName) {
  try {
    await clearCollection(colName);
  } catch (e) {
    console.log(`Custom collection clean skipped for: ${colName} (Empty or non-existent)`);
  }
}
